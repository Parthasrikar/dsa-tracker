...
export default async function FriendsPage() {
  ...
  // Fetch current user with populated friends
  const user = await User.findById(session.user.id).populate('friends', 'name email image').lean();
  
  // Fetch pending incoming requests
  const pendingRequests = await FriendRequest.find({
    to: session.user.id,
    status: 'PENDING'
  }).populate('from', 'name email image').lean();
