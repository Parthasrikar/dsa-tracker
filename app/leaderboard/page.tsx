...
export default async function LeaderboardPage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
    ...
    // 2. Fetch User-Specific Friend Data
    await dbConnect();
    const [currentUser, outgoingRequests] = await Promise.all([
        User.findById(session.user.id).select('friends').lean(),
        FriendRequest.find({ from: session.user.id, status: 'PENDING' }).select('to').lean()
    ]);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const friendIds = new Set((currentUser as any)?.friends?.map((id: any) => id.toString()) || []);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pendingRequestIds = new Set(outgoingRequests.map((r: any) => r.to.toString()));
